 ...
 *
 * bin/install-wp-tests-sqlite.sh installs the drop-in as part of setup, so
 * this function is a no-op when that script has already run.  It remains here
 * as a fallback for environments where only `composer install` was run and the
 * shell setup script was not executed.
 */
function _relevanssi_light_install_sqlite_dropin() {
	global $_tests_dir;

	$plugin_dir  = dirname( __DIR__ ) . '/vendor/wordpress/sqlite-database-integration';
	$_local_tests_dir = getenv( 'WP_TESTS_DIR' );
	if ( ! $_local_tests_dir ) {
		$_local_tests_dir = dirname( __DIR__ ) . '/.wp-test/wordpress-tests-lib';
	}
	$wp_core_dir = rtrim( getenv( 'WP_CORE_DIR' ) ?: ( dirname( $_local_tests_dir ) . '/wordpress/' ), '/' );
